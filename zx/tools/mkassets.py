...
def main(argv):
    ...
    with open(os.path.join(out, 'assets.inc'), 'w') as f:
        ...
        f.write('BANK_SPR    equ %d' % BANK_SPR + chr(10))
    ...
    print('bank_art    %d байт (комната %d + маска %d), банк %d'
          % (len(screen) + len(fore), len(screen), len(fore), BANK_ART))
    print('bank_spr    %d байт, %d кадров, банк %d  (свободно %d)'
          % (len(blob), len(used), BANK_SPR, 0x4000 - len(blob)))
