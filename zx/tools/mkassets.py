...
def level_head(n, chset_len=0, nxt=(0, 0)):
    ...
    return (kid_place(block, face) + bytes([1 if n < LEVELS else 0])
            ...
            + bytes(2))     # LH_CUT: build.sh puts the scene's length in


LEVEL_HEAD = 16
LH_NEXT, LH_NEXTLEN, LH_INK, LH_SET, LH_CUT = 8, 10, 12, 13, 14

# The princess's scenes TOPCTRL.S plays before certain levels, those this
...
def main(argv):
    ...
    # where build.sh puts the length of the scene the next level begins
    # with, once it has assembled it: the head of the level before
    cutheads = []
    if START_LEVEL + 1 in CUT_BEFORE and START_LEVEL < LEVELS:
