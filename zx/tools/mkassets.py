...
def main(argv):
    ...
    with open(os.path.join(out, 'assets.inc'), 'w') as f:
        ...
        f.write('room        equ %d' % (PAGE_WINDOW + len(art)) + chr(10))
        f.write('floormask   equ %d' % PAGE_WINDOW + chr(10))
        f.write('halfmask    equ %d' % (PAGE_WINDOW + len(fmask)) + chr(10))
        f.write('foremask    equ %d'
                % (PAGE_WINDOW + len(art) + len(screen)) + chr(10))
