...
def main(argv):
    ...
    maskcan = tables + len(spare) + 3
    assert maskcan + 2 * 40 * 45 <= 0x10000, 'the canvas bank is full'
    spr3 = blobs[2] if len(blobs) > 2 else b''
    canvas = PAGE_WINDOW + (len(spr3) + len(SIG_SPR) if spr3 else 0) + 3
    assert canvas + 40 * 192 <= 0x10000, 'no room for the canvas'
